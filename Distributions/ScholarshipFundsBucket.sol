...
/// @title A flexible approach to token distribution from a manually allocated scholarship funds bucket.
contract ScholarshipFundsBucket is IDistribution {
    using SafeMath for uint;
                
    Token public SONAToken;
    
    address public constant ACCOUNT_OF_SCHOLARSHIP_FUNDS = //INSERT ACCOUNT ADDRESS OF SCHOLARSHIP_FUNDS;
    ...
    function flexDistribution(address[] calldata _recipients, uint[] calldata _amounts) external {
        require(msg.sender == ACCOUNT_OF_SCHOLARSHIP_FUNDS, "[!] The transaction call was initiated by the wrong account");
