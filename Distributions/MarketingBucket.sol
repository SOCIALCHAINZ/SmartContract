...
/// @title A flexible approach to token distribution from a manually allocated marketing bucket.
contract MarketingBucket is IDistribution {
    using SafeMath for uint;
                
    Token public SONAToken;
    
    address public constant ACCOUNT_OF_MARKETING = //INSERT ACCOUNT ADDRESS OF MARKETING;
    ...
    function flexDistribution(address[] calldata _recipients, uint[] calldata _amounts) external {
        require(msg.sender == ACCOUNT_OF_MARKETING, "[!] The transaction call was initiated by the wrong account");
