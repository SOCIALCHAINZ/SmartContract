...
/// @title A flexible approach to token distribution from a manually allocated founders bucket.
contract FoundersBucket is IDistribution {
    using SafeMath for uint;
                
    Token public SONAToken;
    
    address public constant ACCOUNT_OF_FOUNDERS = //INSERT ACCOUNT ADDRESS OF FOUNDERS;
    ...
    function flexDistribution(address[] calldata _recipients, uint[] calldata _amounts) external {
        require(msg.sender == ACCOUNT_OF_FOUNDERS, "[!] The transaction call was initiated by the wrong account");
