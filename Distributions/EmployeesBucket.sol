...
/// @title A flexible approach to token distribution from a manually allocated employees bucket.
contract EmployeesBucket is IDistribution {
    using SafeMath for uint;
                
    Token public SONAToken;
    
    address public constant ACCOUNT_OF_EMPLOYEES = //INSERT ACCOUNT ADDRESS OF EMPLOYEES;
    ...
    function flexDistribution(address[] calldata _recipients, uint[] calldata _amounts) external {
        require(msg.sender == ACCOUNT_OF_EMPLOYEES, "[!] The transaction call was initiated by the wrong account");
