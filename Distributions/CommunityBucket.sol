...
/// @title A flexible approach to token distribution from a manually allocated community bucket.
contract CommunityBucket is IDistribution {
    using SafeMath for uint;
                
    Token public SONAToken;
    
    address public constant ACCOUNT_OF_COMMUNITY = //INSERT ACCOUNT ADDRESS OF COMMUNITY;
    ...
    function flexDistribution(address[] calldata _recipients, uint[] calldata _amounts) external {
        require(msg.sender == ACCOUNT_OF_COMMUNITY, "[!] The transaction call was initiated by the wrong account");
