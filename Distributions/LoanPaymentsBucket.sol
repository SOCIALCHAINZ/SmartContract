...
/// @title A flexible approach to token distribution from a manually allocated loan payments bucket.
contract LoanPaymentsBucket is IDistribution {
    using SafeMath for uint;
                
    Token public SONAToken;
    
    address public constant ACCOUNT_OF_LOAN_PAYMENTS = //INSERT ACCOUNT ADDRESS OF LOAN_PAYMENTS;
    ...
    function flexDistribution(address[] calldata _recipients, uint[] calldata _amounts) external {
        require(msg.sender == ACCOUNT_OF_LOAN_PAYMENTS, "[!] The transaction call was initiated by the wrong account");
